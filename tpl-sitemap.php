            ...
            <section id="primary" class="<?php echo $page_layout;?>">
                ...
                <?php wp_link_pages( array('before'=>'<div class="page-link">',	'after'=>'</div>', 'link_before'=>'<span>', 'link_after'=>'</span>', 'next_or_number'=>'number',
                 ...
                 <div class="column one-half">
                    <h4><?php _e('Pages',IAMD_TEXT_DOMAIN);?></h4>
                    <ul class="sitemap"><?php wp_list_pages('title_li='); ?></ul>
        
                    <div class="hr-invisible"> </div>
                    <h4><?php _e('Authors',IAMD_TEXT_DOMAIN);?></h4>
                    <ul class="sitemap"><?php wp_list_authors('show_fullname=1&optioncount=1&exclude_admin=0'); ?></ul>
                    
                 </div>
                 
                 <div class="column one-half last">
                    <h4><?php _e('Categories',IAMD_TEXT_DOMAIN);?></h4>
                    <ul class="sitemap"><?php wp_list_categories('title_li='); ?></ul>
                    
                    <div class="hr-invisible"> </div>
                    <h4><?php _e('Latest Posts',IAMD_TEXT_DOMAIN);?></h4>
                    <ul class="sitemap"><?php wp_get_archives(array('type'=>'postbypost','limit'=>20));?></ul>
                    
                     <div class="hr-invisible"> </div>
                     <h4><?php _e('Archives',IAMD_TEXT_DOMAIN);?></h4>
                     <ul class="sitemap"><?php wp_get_archives(array('type'=>'monthly','limit'=>12));?></ul>
        
                     <div class="hr-invisible"> </div>
                     <h4><?php _e('Tags',IAMD_TEXT_DOMAIN);?></h4>
                     <ul class="sitemap"><?php $tags = get_tags();
                           if($tags):
                                foreach ($tags as $tag): 	echo '<li><a href="'.get_tag_link($tag->term_id).'">'.$tag->name.'</a></li>'; endforeach;
                           endif;?></ul>
