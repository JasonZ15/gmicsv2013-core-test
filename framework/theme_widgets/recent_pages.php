...
class MY_Recent_Pages extends WP_Widget {
	...
	function MY_Recent_Pages() {
		...
		$this->WP_Widget(false,IAMD_THEME_NAME.__(' Pages',IAMD_TEXT_DOMAIN),$widget_options);
	}
	...
	function form($instance) {
	    ...
	    <p><label for="<?php echo $this->get_field_id('_pages'); ?>">
			<?php _e('Choose the pages you want to display (multiple selection possible)',IAMD_TEXT_DOMAIN);?></label>
            <select class="widefat" id="<?php echo $this->get_field_id('_pages').'[]';?>" 
            	name="<?php echo $this->get_field_name('_pages').'[]';?>" multiple="multiple">
                <option value=""><?php _e("Select Page",IAMD_TEXT_DOMAIN);?></option>
...
<?php
	...
	function widget($args, $instance) {
		extract($args);
		global $post;
		#$title = empty($instance['title']) ? 'Recent Pages' : apply_filters('widget_title', $instance['title']);
		$title = empty($instance['title']) ?'' : apply_filters('widget_title', $instance['title']);
		#$p = is_array($instance['_pages']) ? array_filter($instance['_pages']) : array();
		#$_pages = !empty($p)? $p:"";
		$_pages = "";
			 ...
			 while(have_posts()):
				...
				echo "<li>";
					...
					if($show_title):
						...
						echo "<h6>{$title}</h6>";
						#echo mytheme_excerpt('mytheme_excerptlength_teaser1', 'mytheme_excerptmore');
						echo mytheme_excerpt();
					endif;
				echo "</li>";
			 endwhile;
			 else:
				echo "<li><h6>".__('No Pages found',IAMD_TEXT_DOMAIN)."</h6></li>";			 
			 endif;
			 wp_reset_query();
	 	echo "</ul></div>";			 
		echo $after_widget;
